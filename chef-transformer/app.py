...
def main():
    ...
    with col2:
        st.image(load_image_from_local("asset/images/chef_logo.jpg"), width=300)

    with col1:
        st.markdown(meta.HEADER_INFO, unsafe_allow_html=True)

        st.markdown(meta.CHEF_INFO, unsafe_allow_html=True)
        chef = st.selectbox("Choose your chef", index=0, options=["Chef A", "Chef B"])
    ...
    if recipe_button:
        ...
        with st.spinner("Generating recipe..."):
            ...
            if not isinstance(items, str) or not len(items) > 1:
                ...
                gen_kw = chef_top if chef == "Chef A" else chef_beam
